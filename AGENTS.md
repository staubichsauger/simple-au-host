...
- `HostModeRootView.swift` is the top-level mode switch. The app has two user-facing paths:
  - **Simple mode**: single input channel -> optional single inserted Audio Unit -> single output channel.
  - **Multi track mode**: multiple mono/stereo tracks, each with its own routing, plugin selection, and latency class.
...
- `MultiTrackViewModel` builds a `MultiTrackHostConfiguration` from global device settings plus a dynamic list of enabled tracks. It is responsible for track sanitization, per-track routing validation, and validation of the buffered/broadcast internal buffer sizes.
...
- Each enabled track becomes a private `TrackRuntime` instance that owns:
  - its own plugin instance (if any),
...
- Prefer changing the view models when adjusting validation, persistence, or UI-derived configuration; prefer changing the controllers only for realtime audio behavior, device routing, or Audio Unit hosting.
